...
#'
#' 1. A binomial random-ligation model, where pair probability is derived from
#'    per-gene/transcript counts and tested using the right-tail binomial test.
#' 2. An odds-ratio / Fisher exact test based on a 2x2 contingency table using
#'    chimeric counts and singleton counts. This test is used in RBP-dependent and
#'    RNA probing methods like RIL-seq
#'
#' The function also outputs abundance fractions for each duplex pair and counts
#' of other RNA duplexes formed by either or neither gene/transcript in the pair.
#'
#' @param gi `GInteractions` object annotated with gene/transcript names.
#' @param df_counts `data.frame`. A two-column data frame with gene/transcript
#'   counts. The first column should match the `<id_col>.A` / `<id_col>.B`
#'   identifiers in `gi`. The second column is the corresponding count.
#' @param id_col `character(1)`. Prefix for gene/transcript metadata fields in
#'   `gi`. Two fields, `<id_col>.A` and `<id_col>.B`, are expected.
#' @param compute_binom `logical(1)`. Whether to compute the binomial
#'   random-ligation test. Default is `TRUE`.
#' @param compute_odds `logical(1)`. Whether to compute the odds-ratio /
#'   Fisher exact test. Default is `TRUE`.
#'
#' @return `GInteractions` object with added metadata columns.
#'
#' @details
#' **Binomial model**
#'
...
#'
#' Alternative hypothesis: the duplex is enriched above the random-ligation
#' expectation.
#'
#' The random-ligation probability \eqn{P(a, b)} is defined as:
#'
#' \deqn{
#' P(a, b) \propto
#' \begin{cases}
#'     2 \cdot P(a) \cdot P(b) & \text{if } a:b \text{ is observed and } a \neq b \\
#'     P(a)^2                  & \text{if } a:b \text{ is observed and } a = b \\
#'     0                       & \text{otherwise}
#' \end{cases}
#' }
#'
...
#'
#' \deqn{
#' P(a) = \frac{\text{N reads}(a)}{\text{total N reads}}
#' }
#'
#' The observed-pair probabilities are renormalized to sum to one and the
#' p-value is computed from the right tail of the binomial distribution.
#'
...
#'
#' \tabular{lll}{
#'                 \tab B present \tab B absent\cr
#'   A present     \tab C1        \tab C2\cr
#'   A absent      \tab C3        \tab C4\cr
#' }
#'
#' with:
#' \itemize{
#'   \item `C1`: chimeric fragments containing both `A` and `B`
#'   \item `C2`: fragments containing `A` but not `B`
#'   \item `C3`: fragments containing `B` but not `A`
#'   \item `C4`: fragments containing neither `A` nor `B`
#' }
#'
...
calculateLigationPvalues <- function(gi,
    ...
    chim_df <- as_tibble(mcols(gi)) %>%
        dplyr::select(c(n_reads, !!!id_columns))
    ...
    chim_df <- chim_df %>%
        group_by(AB) %>%
        summarise(n_reads_chim_total = sum(n), .groups = "drop")
    ...
    chim_df <- chim_df %>%
        left_join(
            tibble(idA = df_all_cts$RNA, nA = df_all_cts$n),
            by = c("A" = "idA")
        ) %>%
        left_join(
            tibble(idB = df_all_cts$RNA, nB = df_all_cts$n),
            by = c("B" = "idB")
        ) %>%
        dplyr::filter((A %in% unique_ids) & (B %in% unique_ids))

    N_chim_total <- sum(chim_df$n_reads_chim_total)
    N_total <- sum(df_all_cts$n)
    N_frag_total <- N_total + N_chim_total

    # pair-level abundance
    chim_df <- chim_df %>%
        mutate(
            Pa = nA / N_total,
            Pb = nB / N_total,
            Pab = ifelse(A != B, 2 * Pa * Pb, Pa * Pa)
        )
    ...
    chim_df$nonchimB <- vapply(
        vctrs::vec_cast(chim_df$B, character()),
        function(x) {
            df_all_cts %>%
                dplyr::filter(RNA == x) %>%
                pull(n)
    ...
    # unify output
    chim_df$pval <- NA_real_
    chim_df$p.adj <- NA_real_
    chim_df$odds_ratio <- NA_real_
    chim_df$odds_pval <- NA_real_
    chim_df$odds_p.adj <- NA_real_

    if (compute_binom) {
        chim_df$Pab_norm <- chim_df$Pab / sum(chim_df$Pab)

        chim_df$pval <- pbinom(
            chim_df$n_reads_chim_total - 1,
            size = N_chim_total,
        ...
        chim_df$Pab_norm <- NA_real_
    }

    if (compute_odds) {
        # 2x2 table - odds test similar to the one in bacterial methods :
        #            B present                  B absent
        # A present  C1 = AB chimera           C2 = A only
        # A absent   C3 = B only               C4 = neither A nor B
        ...
        odds_res <- dplyr::bind_rows(odds_res)
        chim_df$odds_ratio <- odds_res$odds_ratio
        chim_df$odds_pval <- odds_res$odds_pval
    ...
    statcounts_final <- tibble(chim_id = seq_len(length(gi))) %>%
        ...
        dplyr::select(-c(chim_id, AB)) %>%
        data.frame()

    mcols_old <- mcols(gi) %>%
        data.frame() %>%
        as_tibble() %>%
        dplyr::select(-any_of(colnames(statcounts_final))) %>%
        data.frame()
